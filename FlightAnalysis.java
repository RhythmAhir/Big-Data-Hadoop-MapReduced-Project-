...
public class FlightAnalysis {
	public static void main(String[] args) throws Exception {
		...
		job1.setJarByClass(FlightAnalysis.class);
		job1.setMapperClass(MapperOnSchedule.class);
		job1.setReducerClass(ReducerOnSchedule.class);
		...
		if (job1.waitForCompletion(true)) {
			...
			job2.setJarByClass(FlightAnalysis.class);
			job2.setMapperClass(MapperFlightTaxiTime.class);
			job2.setReducerClass(ReducerFlightTaxiTime.class);
			...
			if (job2.waitForCompletion(true)) {
				...
				job3.setJarByClass(FlightAnalysis.class);
				job3.setMapperClass(MapperFlightCancel.class);
				job3.setReducerClass(ReducerFlightCancel.class);
...
//mapper class for flight schedule ontime
class MapperOnSchedule extends Mapper<LongWritable, Text, Text, Text> {
		...
		protected void map(LongWritable key, Text value, Mapper<LongWritable, Text, Text, Text>.Context context)
			...
			int delay=0;
			if (line[14].trim().matches("-?[0-9]+") || line[15].trim().matches("-?[0-9]+")) {
				if(line[15].trim().matches("-?[0-9]+")) {
					delay = Integer.valueOf(line[14]);
				}
				if(line[14].trim().matches("-?[0-9]+"))
				 delay = delay+Integer.valueOf(line[14]);
	...
	// Reducer class for flight schedule ontime
 class ReducerOnSchedule extends Reducer<Text, Text, Text, Text> {
...
class MapperFlightTaxiTime extends Mapper<LongWritable, Text, Text, Text> {
...
class ReducerFlightTaxiTime extends Reducer<Text, Text, Text, Text> {
	...
	}//end of reducer calss for taxi time

 class MapperFlightCancel extends Mapper<LongWritable, Text, Text, Text> {
...
// Reducer Three
class ReducerFlightCancel extends Reducer<Text, Text, Text, Text> {
